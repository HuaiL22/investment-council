...
- Screening runs record theme or comparison label, market, horizon, liquidity requirement, and child candidates. When the user says “U.S.-listed” or “美股” without narrower venue instructions, `state_store.py init` records Nasdaq, NYSE, and NYSE American common shares and direct ADR/ADS as the default eligible boundary, with OTC and funds excluded unless explicitly overridden. When a theme horizon is absent, initialization records the disclosed 3–12 month default.
...
`initialized -> evidence_frozen -> council_running -> decision_ready -> complete|failed`
...
`assemble_report.py` validates all pre-completion stages. After it succeeds, commit `complete_report.md` as the complete stage and run verification.
